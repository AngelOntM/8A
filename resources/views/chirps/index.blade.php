        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ URL::signedRoute('chirps.store') }}">
                        @csrf
                    ...
                    <div class="p-6 flex space-x-2">
                        ...
                        @if (Auth::user()->rol_id == 1 || Auth::user()->rol_id == 2)
                            <x-dropdown>
                                ...
                                <x-slot name="content">
                                    <x-dropdown-link :href="URL::signedRoute('chirps.edit', $chirp)">
                                        {{ __('Edit Chirp') }}
                                    </x-dropdown-link>
                                    <form method="POST" action="{{ URL::signedRoute('chirps.destroy', $chirp) }}">
                                        @csrf @method('DELETE')
                                        <x-dropdown-link :href="URL::signedRoute('chirps.destroy', $chirp)" onclick="event.preventDefault(); this.closest('form').submit();">
                                            {{ __('Delete Chirp') }}
                                        </x-dropdown-link>
                                    </form>
